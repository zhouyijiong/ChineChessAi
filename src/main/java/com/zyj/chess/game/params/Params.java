...
import com.zyj.chess.game.tool.KeyVal;
...
public final class Params {
    ...
    public static final long[] DIGITS = {1000000000000000000L, 100000000000000000L, 10000000000000000L, 1000000000000000L, 100000000000000L, 10000000000000L, 1000000000000L, 100000000000L, 10000000000L, 1000000000L, 100000000L, 10000000L, 1000000L, 100000L, 10000L, 1000L, 100L, 10L, 1L};

    public static final Map<Integer, KeyVal<List<Integer>, List<Integer>>> HORSE_NAVIGATE;
    public static final Map<Integer, KeyVal<List<Integer>, List<Integer>>> BLACK_ELEPHANT_NAVIGATE;
    public static final Map<Integer, KeyVal<List<Integer>, List<Integer>>> RED_ELEPHANT_NAVIGATE;
    ...
    public static void getNavigate(Map<Integer, KeyVal<List<Integer>, List<Integer>>> maps, List<Integer> list, int x, int y) {
        KeyVal<List<Integer>, List<Integer>> kv = maps.get(y * 10 + x);
        List<Integer> point = kv.k;
        List<Integer> baffle = kv.v;
    ...
    private static Map<Integer, KeyVal<List<Integer>, List<Integer>>> calcHorseNavigate() {
        Map<Integer, KeyVal<List<Integer>, List<Integer>>> map = new HashMap<>();
        for (int[] arrays : BOARD_POINT) {
            for (int i : arrays) {
                ...
                map.put(i, KeyVal.init(point, baffle));
            }
        }
        return map;
    }

    private static Map<Integer, KeyVal<List<Integer>, List<Integer>>> calcElephantNavigate() {
        Map<Integer, KeyVal<List<Integer>, List<Integer>>> map = new HashMap<>();
    ...
    public static void calcCannonNavigate(List<Integer> list, int id, int x, int y) {
        boolean key = id < 8;
        for (id = x + 1; id < 10; ++id)
            if (calcCannonMoveCore(list, id, y)) for (++id; id < 10; ++id) if (calcAttackCore(list, id, y, key)) break;
        for (id = x - 1; id > 0; --id)
            if (calcCannonMoveCore(list, id, y)) for (--id; id > 0; --id) if (calcAttackCore(list, id, y, key)) break;
        for (id = y + 1; id < 11; ++id)
            if (calcCannonMoveCore(list, x, id)) for (++id; id < 11; ++id) if (calcAttackCore(list, x, id, key)) break;
        for (id = y - 1; id > 0; --id)
    ...
    private static boolean calcCareNavigateCore(Navigate navigate, int x, int y, boolean key) {
        ...
        if ((id = Game.BOARD.get(y, x)) == 0) {
            ...
            return false;
        } else if (key ? id > 7 : id < 8) navigate.eat.put(y * 10 + x, null);
        return true;
    }
    ...
    private static Map<Integer, KeyVal<List<Integer>, List<Integer>>> mirrorElephantNavigate() {
        Map<Integer, KeyVal<List<Integer>, List<Integer>>> mirrorMap = new HashMap<>();
        for (Map.Entry<Integer, KeyVal<List<Integer>, List<Integer>>> item : BLACK_ELEPHANT_NAVIGATE.entrySet()) {
            KeyVal<List<Integer>, List<Integer>> kv = item.getValue();
            mirrorMap.put((11 - item.getKey() / 10) * 10 + item.getKey() % 10, KeyVal.init(mirrorList(kv.k), mirrorList(kv.v)));
        }
        return mirrorMap;
    }
}